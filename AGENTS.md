...
SketchUp 插件，用于装修用量统计。扫描 SU 模型面/容器，按组件层级 / 空间 / 部位 / 材料分组，输出几何量报表。支持四种计量方式：**面积（m²）/ 长度（m）/ 体积（m³）/ 件数（个）**，由 `TakeoffPolicy` 3 档优先级决议，每个量纲背后由独立的 `Strategy` 类承担"如何聚合/如何从容器产出/默认单位"。前端仅保留按组件树视图。

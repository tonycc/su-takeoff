...
> ⚠️ **设计已转向（2026-07-30）**：本文档描述的是最初的**材质级**关联方案（`MaterialMapping` + 材料映射页「平台SKU」列 + 材料汇总行选择）。经与用户进一步确认，最终实现改为**组件级**关联：新增独立的 `ComponentSkuMapping`（按组件定义名，**不参与算量**），选择入口在模型视图的**组件行**「产品信息」列；材质级方案的相关改动已回退。以下为原始设计存档，仅供追溯。

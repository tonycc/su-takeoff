...
SketchUp 插件 — 装修用量几何统计工具。扫描 SU 模型面与容器，按组件层级 / 空间 / 部位 / 材料分组，支持**面积（m²）/ 长度（m）/ 体积（m³）/ 件数（个）**四种计量方式，由 `TakeoffPolicy` 4 档优先级自动决议。
...
│   ├── calculator.rb          # 计算引擎（面积/长度/体积/件数）
...
单测覆盖数据模型、计算器、策略决议、映射表，均独立于 SU 运行时。Scanner / Dialog 需在 SketchUp 内手动验证。

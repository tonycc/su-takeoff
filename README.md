...
- **项目绑定与云端推送**：支持平台账号登录、项目绑定、SU v2 payload 构建、后台推送、失败重试和本地 outbox
- **几何启发式**：自动识别窄长垂直面为线材（宽 ≤ 0.2m + 长宽比 > 15），结果以橙色边框标记待用户确认，阈值可在参数管理页调整
...
│           ├── system_management.js # 系统管理页（登录信息/退出登录）
│           └── settings.js    # 参数管理页（标签/分类/启发式/阈值）
├── data/
...
3. 登录成功后进入功能页面：
   ...
   - **参数管理**：组件分类单位、算量标签定义（支持多选组合）、启发式开关+阈值
   - **项目绑定与云端推送**：登录平台账号、绑定项目、推送 SU v2 算量 payload
   - **系统管理**：查看软件版本、登录账号、租户、推送权限和退出登录
4. 摘要条显示总览统计
